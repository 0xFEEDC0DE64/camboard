...
constexpr auto SSD1306_ADDRESS = 0x3c;
constexpr auto I2C_SDA = 21;
constexpr auto I2C_SCL = 22;

constexpr auto BUTTON_1 = 34;

constexpr auto Y9_GPIO_NUM = 39;
constexpr auto Y8_GPIO_NUM = 36;
constexpr auto Y7_GPIO_NUM = 23;
constexpr auto Y6_GPIO_NUM = 18;
constexpr auto Y5_GPIO_NUM = 15;
constexpr auto Y4_GPIO_NUM = 4;
constexpr auto Y3_GPIO_NUM = 14;
constexpr auto Y2_GPIO_NUM = 5;
constexpr auto VSYNC_GPIO_NUM = 27;
constexpr auto HREF_GPIO_NUM = 25;
constexpr auto PCLK_GPIO_NUM = 19;
constexpr auto PWDN_GPIO_NUM = 26;
constexpr auto RESET_GPIO_NUM = -1;
constexpr auto XCLK_GPIO_NUM = 32;
constexpr auto SIOD_GPIO_NUM = 13;
constexpr auto SIOC_GPIO_NUM = 12;
...
void drawFrame1(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->drawString(64 + x, 5 + y, "Status:");
    ...
    display->drawString(64 + x, 20 + y, status);
}
...
void drawFrame2(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->drawString(64 + x, 5 + y, "MAC-Address:");
    display->drawString(64 + x, 20 + y, WiFi.macAddress());
}
...
void drawFrame3(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->drawString(64 + x, 5 + y, "IP-Address:");
    display->drawString(64 + x, 20 + y, WiFi.localIP().toString());
}
...
void drawFrame4(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->drawString(64 + x, 5 + y, "Hostname:");
    display->drawString(64 + x, 20 + y, WiFi.getHostname());
}

std::array<FrameCallback, 4> frames { drawFrame1, drawFrame2, drawFrame3, drawFrame4 };

bool autoTransitionEnabled = true;
...
static esp_err_t index_handler(httpd_req_t *req){
    ...
    return httpd_resp_send(req, reinterpret_cast<const char *>(&index_html_gz[0]), index_html_gz.size());
}
...
void setup()
{
...

    {
        ...
        if (error != ESP_OK)
        {
            ...
            oled.clear();
            oled.drawString(oled.getWidth() / 2, oled.getHeight() / 2, "Error camera");
            oled.drawString(oled.getWidth() / 2, (oled.getHeight() / 2) + 10, String(error));
            oled.display();
            while (true);
        }
    }
...

    {
...

        {
...

            {
                ...
                if (error != ESP_OK)
                {
                    Serial.println("error initializing webserver");
                    Serial.println(error);

                    oled.clear();
                    oled.drawString(oled.getWidth() / 2, oled.getHeight() / 2, "Error webserver");
                    oled.drawString(oled.getWidth() / 2, (oled.getHeight() / 2) + 10, String(error));
                    oled.display();
                    while (true);
                }
            ...
            for (const httpd_uri_t &uri : {
                 ...
                 httpd_uri_t { "/control", HTTP_GET, cmd_handler,     NULL },
                 httpd_uri_t { "/capture", HTTP_GET, capture_handler, NULL }
            })
            {
                ...
                if (error != ESP_OK)
                {
                    ...
                    oled.clear();
                    oled.drawString(oled.getWidth() / 2, oled.getHeight() / 2, "Error webserver uri");
                    oled.drawString(oled.getWidth() / 2, (oled.getHeight() / 2) + 10, uri.uri);
                    oled.drawString(oled.getWidth() / 2, (oled.getHeight() / 2) + 20, String(error));
                    oled.display();
                    while (true);
                }
            }
...

        {
...

            {
                ...
                if (error != ESP_OK)
                {
                    Serial.println("error initializing streamserver");
                    Serial.println(error);

                    oled.clear();
                    oled.drawString(oled.getWidth() / 2, oled.getHeight() / 2, "Error streamserver");
                    oled.drawString(oled.getWidth() / 2, (oled.getHeight() / 2) + 10, String(error));
                    oled.display();
                    while (true);
                }
            ...
            for (const httpd_uri_t &uri : {
                 httpd_uri_t { "/stream", HTTP_GET, stream_handler, NULL }
            })
            {
                const esp_err_t error = httpd_register_uri_handler(webserver, &uri);
                if (error != ESP_OK)
